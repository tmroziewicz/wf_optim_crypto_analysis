...
smooth_matrix <- function( matx.df, stat.name ) {
  print(head(matx.df))
  print(stat.name)
  matx.df <- dcast(matx.df,  wf.test_length ~ wf.train_length,value.var = stat.name)
  ...
  dflist <- list()
  colnames(matx.df) <- seq(1:ncol(matx.df))
  rownames(matx.df) <-seq(1:nrow(matx.df))
  ...
  for (col in 1:ncol(matx.df)) {
    for (row in 1:nrow(matx.df)) {
      ...
      dflist <- append(dflist, list(data.frame(wf.test_length = rowname.map.df[row,] , wf.train_length = colname.map.df[col,] ,  weighted = weighted)))
    }
  }
