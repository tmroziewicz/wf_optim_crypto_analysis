...
df_plot <- merged_cumsum_xts %>%
  ...
  mutate(
    # Assign Alpha: 'Buy And Hold' gets extra, others get base alpha
    Alpha_Val = ifelse(grepl("Buy And Hold", Series), alpha + alphaadd, alpha)
  )

# 2. Define the Linetype mapping (BTC = solid, ETH = dashed, BNB = dotted)
unique_series <- unique(df_plot$Series)
line_map <- setNames(
  case_when(
    grepl("BTC", unique_series) ~ "solid",
    grepl("ETH", unique_series) ~ "dashed",
    grepl("BNB", unique_series) ~ "dotted",
    TRUE ~ "solid"
  ),
  unique_series
)
...
ggplot(df_plot, aes(x = Date, y = Value, color = Series, linetype = Series)) +
  geom_line(aes(alpha = Alpha_Val), size = 0.7) +
  
  # Set manual scales
  scale_linetype_manual(values = line_map) +
  scale_alpha_identity() +
  
  # Transformation and Labels
  coord_trans(y = 'log') +
  labs(x = "Unseen data period", y = "Cumulative return", caption = str_wrap(paste("Note:...", unseen_test_description_str), 80)) +
  
  # Theme and Legend Fixes
  theme_bw() +
  theme(
    panel.border = element_blank(),
    legend.position = "top",           # Move to top to avoid overlapping data
    legend.title = element_blank(),
    legend.text = element_text(size = 7), # Slightly smaller text to prevent clipping
    legend.key.width = unit(1.2, "cm"), # Make legend keys wider so dashes/dots are visible
    plot.margin = margin(t = 10, r = 20, b = 10, l = 10) # Add right margin padding
  ) +
  # Use 3 columns to ensure all text fits horizontally
